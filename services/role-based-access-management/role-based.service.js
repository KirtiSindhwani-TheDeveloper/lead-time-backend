...
module.exports = {
  ...
  uploadRoleFormat: async function (req, filePath) {
    req = JSON.parse(req);
    // console.log("req ",req);
    try {
      ...
      let publicIp = "Fetching public IP...";
      publicIp = await getPublicIp();
      let query4 = `Insert into role_master(role_name,createdby,status
          ,SIMS
          ,AUDIT
          ,GAINER
          ,IT
      ...
      for (let item of excelData) {
        ...
        let getParentIdQuery = `select parentId from module_master where module_name=@subModule`;
        ...
        let query = `select  bv.id as businessVerticalId ,mm.id as pageId  from module_master mm join business_vertical_master bv
        ...
        let query234 = `Insert into Audit_log(roleName,userID,status
               ,SIMS
               ,AUDIT
               ,GAINER
               ,IT
