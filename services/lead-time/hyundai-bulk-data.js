...
module.exports = {
    bulkInsertPOData: async function(data,pool, dealer, location,res) {
      ...
      const Location = location || item["location"]; 
            return [  item["order no"], // Order No (lowercase)
              ...
              item['po date'] !== "0-00-00" ? convertExcelSerialToIST(parseFloat(item['po date']),item) : null, // PO DATE (lowercase)
    ...
    bulkInsertData:async function(data,pool,dealer,location,res){
     ...
     // console.log("result in hyundai ",result)
      ...
      item['invoice date'] !== "0-00-00" ? convertExcelSerialToIST(parseFloat(item['invoice date']), item) : null, // Invoice Date (lowercase)
      item['gr date'] !== "0-00-00" ? convertExcelSerialToIST(parseFloat(item['gr date']), item) : null, // GR Date (lowercase)
